...
### Added
- **MCP 智能能力增强**：新增 `qm_capability_map`、`qm_tool_help`、`qm_health_report`、`qm_find_entities` 四个只读工具，让 agent 可以自发现工具目录、查看单工具 schema、生成综合健康报告，并跨 Bot/持仓/订单/设置/日志检索关键对象；设置搜索结果继续对 token、secret、password、webhook 等敏感字段脱敏。
- **左侧菜单搜索**：桌面展开和移动抽屉模式显示菜单搜索框，收起模式提供搜索图标一键展开并聚焦；命中菜单后直接跳转目标页面并清空搜索。
- 新增 `PRODUCT_OVERVIEW.md`，记录当前产品定位、核心功能、技术栈、部署方式与待办。

### Changed
- 版本号同步前后端到 `3.109.0-rc1`。
...
### Changed
- **WebAuthn 非安全上下文提示加上跳过指引**：rc19 文案只告诉用户「请改用 https:// 或 localhost」，但用户在远程机上没法立即就配证书/隧道，需要先有路用起来。文案改成分场景：首次设置场景额外提示「如果暂不需要指纹，请直接点下方『稍后注册』用密码登录即可」；登录场景额外提示「请直接在上方输入密码登录即可」。24 个语言文件 zh-CN/zh-TW/en-US 真翻译，其余英文 placeholder。
...
### Fixed
- **登录后白屏 → 显示可读错误**：WebUI 此前没有任何 React 错误边界，登录后只要有一个组件渲染抛错（常见于配置未初始化/数据缺失），整棵组件树被卸载，用户看到一片白屏（曾被误判为「没有 dashboard / 只能改 yaml」）。新增应用级 `ErrorBoundary`（`webui/src/components/ErrorBoundary.tsx`，在 `main.tsx` 包裹根组件），渲染异常时显示标题、说明、刷新按钮与可展开的错误详情（并打到 console），文案走 i18n（`error.render_crash_*`，已补 zh-CN/zh-TW/en-US，其余语言回退）。错误边界刻意只用内联样式、不依赖 Chakra，避免主题层自身出错时连错误界面也崩。

### Changed
- 版本号同步前后端到 `3.108.1-rc17`。
...
### Added
- **单元测试覆盖推进到 30%**：新增 `monitor` 新闻风险规则、`notify/aipipe` 配置与静默路径、`arbitrage` 期现对冲、`backtest` tick 撮合 / K 线内回测 / 风控模拟器等单元测试。

### Changed
- Go 全仓语句覆盖率提升到 `30.0%`。
- 版本号同步前后端到 `3.108.1-rc8`。

---

## [3.108.1-rc7] - 2026-06-04

### Added
- **单元测试覆盖推进到 25%**：新增 `backtest` 信号总线与缓存管理、`ai` 配置应用与风险评估、`web` 路由 smoke、`monitor` 看门狗 / 内存管理 / 阈值判断，以及 `notify` 通知构造与消息格式化的单元测试。

### Changed
- Go 全仓语句覆盖率提升到 `25.0%`。
- 版本号同步前后端到 `3.108.1-rc7`。

---

## [3.108.1-rc6] - 2026-06-03

### Added
- **单元测试覆盖推进到 22% 以上**：新增 `agent` 状态机/LLM 请求/工具调用、`agent/tools` 参数校验与建议、`ai/service` 异步任务生命周期与 AIService provider wrapper、`event` 事件消息/去重/价格波动发布，以及 Bitkub / Coins.ph 现货适配器纯逻辑单元测试。

### Changed
- Go 全仓语句覆盖率提升到 `22.1%`。
- 版本号同步前后端到 `3.108.1-rc6`。

---

## [3.108.1-rc5] - 2026-06-03

### Added
- **单元测试覆盖推进到 20% 以上**：新增 `storage` 导出 CSV/JSON/ZIP 与审计日志过滤、`agent` 文件管理器上传/生成/清理、`ai/service` provider 兜底与代理初始化，以及 `backtest` Binance 历史数据下载器 URL/时间解析/本地数据扫描的单元测试。

### Changed
- Go 全仓语句覆盖率提升到 `20.1%`。
- 版本号同步前后端到 `3.108.1-rc5`。

---

## [3.108.1-rc4] - 2026-06-03

### Added
- **单元测试覆盖继续推进**：新增 `agent/types` 安全等级映射、`database` 表名/工厂错误分支、`ai/service` transport 选择与代码块修剪、`plugin` 许可证生成解析/验证/加解密，以及 `profit` 利润提取执行频率判断的单元测试。

### Changed
- 版本号同步前后端到 `3.108.1-rc4`。

---

## [3.108.1-rc3] - 2026-06-03

### Added
- **单元测试覆盖继续推进**：新增 `inspector` 调度器、`arbitrage` 仓位聚合、`saas` 套餐/计费/扩缩容判断，以及 `web` i18n middleware 与参数解析 helper 的单元测试。

### Changed
- 版本号同步前后端到 `3.108.1-rc3`。

---

## [3.108.1-rc2] - 2026-06-03

### Added
- **单元测试覆盖继续增强**：新增 `i18n` 初始化/回退、`lock` 空锁与工厂错误分支、`logger` 日志级别/翻译/错误 hook，以及 `metrics` 基础收集器与 Prometheus recorder smoke 测试。

### Changed
- **测试副作用继续收敛**：`TaskManager` 支持显式设置结果/报告输出目录，Web 回测任务测试改用临时目录，避免异步任务在仓库 `web/backtest` 下遗留结果与报告文件。
- 版本号同步前后端到 `3.108.1-rc2`。

---

## [3.108.1-rc1] - 2026-06-03

### Added
- **单元测试覆盖增强**：新增 `indicators` 技术指标基础计算、波动率指标，以及 `macro` 宏观事件影响分类器的单元测试，覆盖此前 0 覆盖的纯逻辑包。

### Changed
- **CI 测试门禁**：GitHub Actions 新增独立 Test job，PR / push 阶段执行 `go test ./...`、Go 覆盖率摘要与 WebUI `yarn test`。
- **本地测试脚本**：`scripts/tests/run_tests.sh` 从仅跑部分 Go 包扩展为后端全仓单测 + 覆盖率摘要 + 前端 Vitest 单测。
- **测试副作用收敛**：回测报告生成测试改用临时目录，避免单测在仓库 `backtest/reports` 下遗留报告文件。
- 版本号同步前后端到 `3.108.1-rc1`。
...
### Fixed
- **整站配置迁移包**：新增 `GET /api/export/config-migration` 导出完整配置迁移 JSON，包含当前运行时配置、YAML 视图、主库 `app_config` 原始 JSON 与全部 `bot_configs` 文档，便于旧站完整搬迁到新站。
- **迁移导入闭环**：新增 `POST /api/config/import-migration`，导入时优先使用迁移包中的数据库 `app_config` 写入新站主库，并同步运行时配置；同时逐条写入 `bot_configs`，若本地 Bot 文件配置管理器存在也同步落盘。
- **迁移文档**：新增 `docs/CONFIG_MIGRATION.md`，记录导出、导入、导入后检查、回滚与敏感信息处理注意事项。
- **迁移安全提示**：迁移包用于搬站，包含未脱敏配置与密钥，必须按密钥文件管理，不应提交到版本库或公开分享。

---

## [3.105.0-rc41] - 2026-05-23

### Fixed
- **策略生命周期防护**：网格策略停止或未完成 manager 初始化时会忽略价格/订单回调，避免停止后的行情继续触发调单，或异常配置下出现空指针 panic；马丁格尔与趋势跟踪策略的 `Start(ctx)` 会接入新的运行 context，停止时取消当前 context，提升停止/重启一致性。
- **配置写入可靠性**：磁盘 YAML 配置与 Bot 配置改为同目录临时文件写入、同步、再原子替换，并使用 `0600` 权限降低配置中密钥泄露风险。
- **回归测试**：补充网格策略停止态/空 manager 回调测试、马丁/趋势生命周期测试，以及配置原子写入权限与临时文件清理测试。

---

## [3.105.0-rc40] - 2026-05-23

### Fixed
- **API 版本可观测性**：后端统一规范化版本号输出，空白版本会回退为 `unknown`；所有 `/api` 响应在保留 `X-App-Version` 的同时增加 `X-Server-Version` 响应头，便于代理、前端和排障脚本识别服务端版本。
- **回归测试**：补充 `/api/version` 与版本响应头测试，覆盖空白版本兜底和响应头去空白行为。

---

## [3.105.0-rc39] - 2026-05-20

### Fixed
- **Agent 参数工具容错**：`set_parameter`、`validate_parameters`、`suggest_parameters` 对必填参数和对象参数做显式校验，避免 LLM/前端传入缺失或错误类型参数时触发 panic。
- **参数建议稳定性**：参数优化器在未注入 market data 依赖时会使用默认市场数据服务，并对空市场数据结果安全返回，降低 Agent 工具链运行期空指针风险。
- **回归测试**：补充 Agent 参数工具异常输入测试，覆盖缺失参数、错误参数对象和默认优化器依赖路径。

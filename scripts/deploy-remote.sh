...
main() {
    ...
    # 8. 停止旧服务
    log_step "停止旧服务..."
    remote_exec "cd ${REMOTE_PATH} && pkill -f quantmesh || true; sleep 2"
    log_info "✅ 旧服务已停止"
    ...
    if remote_exec "curl -f -s http://localhost:28888/api/status > /dev/null 2>&1 && echo 'ok' || echo 'fail'" | grep -q "ok"; then
        ...
        log_warn "⚠️ 健康检查失败，请查看日志: ${REMOTE_PATH}/logs/quantmesh.log"
    fi
    ...
    log_info "查看日志: ssh ${REMOTE_USER}@${REMOTE_HOST} 'tail -f ${REMOTE_PATH}/logs/quantmesh.log'"
    log_info ""
}

...
无磁盘文件时也可直接启动，通过 **Web** 完成引导。详见 `docs/config-database-design.md`。

### Q3: 如何配置 API 密钥？

**A:** 在 **Web** 或导入 YAML / **`app_config`** 中配置：
...
1. **主配置是否正确**（Web / `app_config` / 导入 YAML）
   ```bash
   # 若使用导入文件：cat my-import.yaml
   ```
...
2. **重新启动**：
   ...
   # 或
   ./quantmesh
   # 或显式传入 YAML：./quantmesh my-import.yaml
   ```
...
### Q22: 持倉安全性检查失败，提示「獲取帳戶信息失败: &lt;APIError&gt; rsp= 」？

**A:** 该错误表示 Binance API 返回了 HTTP 4xx/5xx 且响应体为空，常见原因：

1. **网络/地区限制**
   - Binance.com 在中国大陆、美国等地区受限，可能返回空响应
   - 若需访问主网，请使用代理并设置环境变量：
     ```bash
     export https_proxy=http://127.0.0.1:7890
     export http_proxy=http://127.0.0.1:7890
...
2. **API Key 与网络环境不匹配**
   - 主网 API Key 需连接 `api.binance.com`，测试网需连接 `testnet.binance.vision`
   - 确认主配置中 `testnet` 与 API Key 来源一致（Web / `app_config`）

3. **API Key 无效或权限不足**
   - 在 Binance 后台确认 Key 未过期、未删除
   - 确认已开启「读取」和「现货交易」权限

4. **IP 白名单**
   - 若 API Key 启用了 IP 限制，请将当前出口 IP 加入白名单

5. **快速验证**
   ```bash
   # 测试能否访问 Binance API（无需 Key）

...
主配置权威在数据库表 **`app_config`**；`docs/config/examples/config.example.yaml` 仅作导入模板（文件名可自定）。
...
编辑 `my-import.yaml`，填入你的交易所 API 密钥（或在 Web 引导中填写）：
...
详见 `docs/config-database-design.md`。
...
更多配置说明请参考 `docs/config/examples/config.example.yaml` 文件中的注释。
...
# 运行（无首参时优先从 app_config 加载；或显式传入 YAML）
./quantmesh
...
在 Web 或主配置（**`app_config`** / 导入 YAML）中配置：
...
### 问题 1: 找不到 YAML 或主库无配置

**若启动时传了首参 YAML 路径**，路径必须存在，否则会报错找不到文件。

**若未传首参**，进程从 **`app_config`** 加载；若库为空则进入 Web 引导（内存最小配置，不强制写盘 `config.yaml`）。

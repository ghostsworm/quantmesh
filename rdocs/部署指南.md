...
项目根目录/
├── data/
│   ├── quantmesh.db      # 主数据库（订单、持仓、交易等；含 **app_config** 主配置快照）
│   ├── auth.db          # 认证数据库
│   ├── webauthn.db      # WebAuthn 数据库
│   └── logs.db          # 日志数据库
└── docs/config/examples/config.example.yaml   # 仅作模板/一次性导入；运行时主配置在库中，非固定磁盘 config.yaml
```
...
# 备份所有数据库（logs.db 现在也在 data/ 目录下；主配置在 quantmesh.db 的 app_config）
cp -r data backups/data_$(date +%Y%m%d_%H%M%S)
```
...
- ✅ 部署脚本只编译和替换二进制文件
- ✅ 不会删除或覆盖 `data/` 目录（包含所有数据库文件）
- ✅ 主配置在数据库内更新（不会自动覆盖 data/ 下文件）
...
ExecStart=/path/to/quantmesh/quantmesh
EnvironmentFile=-/path/to/quantmesh/.env
Restart=always
RestartSec=10
...
```ini
[program:quantmesh]
command=/path/to/quantmesh/quantmesh
environment=QUANTMESH_DATABASE_DSN="..."
...
2. 检查导入用 YAML（若使用）及数据目录（**`app_config` 在库内**）：
   ```bash
   ls -lh my-import.yaml 2>/dev/null || true
   ls -lh data/quantmesh.db 2>/dev/null || true
   ```
...
4. 手动运行测试：
   ```bash
   ./quantmesh
   # 或: ./quantmesh /path/to/my-import.yaml
   ```

...
// OSSUploader 阿里云 OSS 上傳器，用於每日上傳审计日志
...
type OSSConfig struct {
	...
	Prefix          string
	UploadTime      string // 每日上傳時间，如 "02:00"
	AuditDir        string
}

// NewOSSUploader 創建 OSS 上傳器
func NewOSSUploader(cfg OSSConfig) (*OSSUploader, error) {
	if cfg.Endpoint == "" || cfg.Bucket == "" || cfg.AccessKeyID == "" || cfg.AccessKeySecret == "" {
		return nil, fmt.Errorf("OSS 配置不完整：endpoint、bucket、access_key_id、access_key_secret 為必填")
	}
	client, err := oss.New(cfg.Endpoint, cfg.AccessKeyID, cfg.AccessKeySecret)
	if err != nil {
		return nil, fmt.Errorf("創建 OSS 客戶端失败: %w", err)
	}
	bucket, err := client.Bucket(cfg.Bucket)
	if err != nil {
		return nil, fmt.Errorf("獲取 Bucket 失败: %w", err)
	}
	if cfg.Prefix == "" {
		cfg.Prefix = "audit/"
	}
...
// UploadDate 上傳指定日期的审计日志文件（格式 2006-01-02）
func (u *OSSUploader) UploadDate(date string) (uploaded int, err error) {
	...
	for _, path := range []string{csvPath, jsonlPath} {
		...
		if e != nil {
			logger.Warn("上傳审计日志失败 %s: %v", path, e)
			continue
		}
		if ok {
			uploaded++
...
func (u *OSSUploader) uploadFile(localPath, date string) (bool, error) {
	...
	logger.Info("✅ 审计日志已上傳 OSS: %s -> %s", localPath, objectKey)
	return true, nil
}
...
func (u *OSSUploader) Start() {
	...
	go u.runScheduler()
	logger.Info("OSS 审计日志每日上傳已啟动，上傳時间: %s", u.uploadTime)
}
	...
	for {
		...
		case now := <-ticker.C:
			if now.Hour() == hour && now.Minute() == min {
				...
				if err != nil {
					logger.Warn("每日审计日志上傳失败: %v", err)
				} else if n > 0 {
					logger.Info("每日审计日志上傳完成: %s 共 %d 個文件", yesterday, n)
				}
			}
		}
	}

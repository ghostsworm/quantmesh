...
const (
	WOOXMainnetWSURL = "wss://wss.woo.org/ws/stream"
	WOOXTestnetWSURL = "wss://wss.staging.woo.org/ws/stream"
)

// WebSocketManager WOO X WebSocket 管理器
...
func (w *WebSocketManager) connect(ctx context.Context, symbol string) {
	for {
		...
		if err != nil {
			logger.Error("WOO X WebSocket dial error: %v", err)
		...
		logger.Info("WOO X WebSocket connected")

		// 认证
		if err := w.authenticate(); err != nil {
			logger.Error("WOO X WebSocket authenticate error: %v", err)
			conn.Close()
			time.Sleep(5 * time.Second)
			continue
		}
		...
		if err := w.subscribe(symbol); err != nil {
			logger.Error("WOO X WebSocket subscribe error: %v", err)
			conn.Close()
			time.Sleep(5 * time.Second)
			continue
		}
		...
		// 连接断开，重连
		logger.Warn("WOO X WebSocket disconnected, reconnecting...")
		time.Sleep(5 * time.Second)
	}
}
...
func (w *WebSocketManager) authenticate() error {
	...
	message := timestamp + "GET/ws/stream"
...
func (w *WebSocketManager) readMessages() {
	...
	for {
		...
		if err != nil {
			logger.Error("WOO X WebSocket read error: %v", err)
			return
		}
...
func (w *WebSocketManager) handleMessage(message []byte) {
	...
	if err := json.Unmarshal(message, &msg); err != nil {
		logger.Error("WOO X WebSocket unmarshal error: %v", err)
		return
	}

	// 处理认证响应
	if event, ok := msg["event"].(string); ok && event == "auth" {
		if success, ok := msg["success"].(bool); ok && success {
			logger.Info("WOO X WebSocket authenticated")
		}
		return
	}
	...
	if event, ok := msg["event"].(string); ok && event == "subscribe" {
		logger.Info("WOO X WebSocket subscribed")
		return
	}
	...
	// 处理订单更新
	if topic, ok := msg["topic"].(string); ok && topic == "executionreport" {
		if data, ok := msg["data"].(interface{}); ok {
			logger.Debug("WOO X WebSocket execution report")
			if w.callback != nil {
				w.callback(data)
			}
		}

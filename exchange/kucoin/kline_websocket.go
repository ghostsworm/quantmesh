...
// KlineWebSocketManager KuCoin K線 WebSocket 管理器
...
func (k *KlineWebSocketManager) Start(ctx context.Context, callback CandleUpdateCallback) error {
	...
	logger.Info("KuCoin K線 WebSocket connected: %s", wsURL)
...
func (k *KlineWebSocketManager) handleMessages(ctx context.Context) {
	defer func() {
		if r := recover(); r != nil {
			logger.Error("KuCoin K線 WebSocket message handler panic: %v", r)
		}
	}()
	...
	for {
		...
		default:
			...
			if err != nil {
				logger.Error("KuCoin K線 WebSocket read error: %v", err)
				k.reconnect(ctx)
				return
			}
...
func (k *KlineWebSocketManager) processMessage(message []byte) {
	...
	if err := json.Unmarshal(message, &baseMsg); err != nil {
		logger.Error("KuCoin K線 unmarshal message error: %v, message: %s", err, string(message))
		return
	}
	...
	switch baseMsg.Type {
	case "welcome":
		logger.Info("KuCoin K線 WebSocket welcome message received")
	case "ack":
		logger.Info("KuCoin K線 WebSocket subscription acknowledged")
	case "pong":
		// 心跳响应
	case "message":
	...
	default:
		logger.Warn("KuCoin K線 unknown message type: %s", baseMsg.Type)
	}
}

// handleCandleUpdate 处理 K線更新
func (k *KlineWebSocketManager) handleCandleUpdate(data json.RawMessage) {
	...
	logger.Debug("KuCoin K線 update: %s, time: %d, open: %.2f, high: %.2f, low: %.2f, close: %.2f, volume: %.2f",
...
func (k *KlineWebSocketManager) ping(ctx context.Context) {
	...
	pingInterval := time.Duration(k.token.PingInterval) * time.Millisecond
	if pingInterval <= 0 {
		logger.Warn("KuCoin K線 WebSocket ping interval 无效(%dms)，回退默认 18s", k.token.PingInterval)
...
func (k *KlineWebSocketManager) reconnect(ctx context.Context) {
	logger.Info("KuCoin K線 WebSocket reconnecting...")
	...
	if callback != nil {
		if err := k.Start(ctx, callback); err != nil {
			logger.Error("KuCoin K線 reconnect error: %v", err)
		}
	}
}
...
func (k *KlineWebSocketManager) Stop() {
	...
	logger.Info("KuCoin K線 WebSocket stopped")
}

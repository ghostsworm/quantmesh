...
// KlineWebSocketManager MEXC K线 WebSocket 管理器
...
func (k *KlineWebSocketManager) connect(ctx context.Context, symbol, interval string) {
	for {
		...
		if err != nil {
			logger.Error("MEXC Kline WebSocket dial error: %v", err)
		...
		logger.Info("MEXC Kline WebSocket connected")

		// 订阅 K线
		if err := k.subscribe(symbol, interval); err != nil {
			logger.Error("MEXC Kline WebSocket subscribe error: %v", err)
			conn.Close()
			time.Sleep(5 * time.Second)
			continue
		}
		...
		// 连接断开，重连
		logger.Warn("MEXC Kline WebSocket disconnected, reconnecting...")
		time.Sleep(5 * time.Second)
	}
}
	...
	for {
		...
		case <-ticker.C:
			...
			if err := k.sendMessage(pingMsg); err != nil {
				logger.Error("MEXC Kline WebSocket ping error: %v", err)
				return
			}
		}
	}
...
func (k *KlineWebSocketManager) readMessages() {
	...
	for {
		...
		if err != nil {
			logger.Error("MEXC Kline WebSocket read error: %v", err)
			return
		}
...
func (k *KlineWebSocketManager) handleMessage(message []byte) {
	...
	if err := json.Unmarshal(message, &msg); err != nil {
		logger.Error("MEXC Kline WebSocket unmarshal error: %v", err)
		return
	}
	...
	// 处理订阅响应
	if method, ok := msg["method"].(string); ok && method == "sub.kline" {
		if code, ok := msg["code"].(float64); ok && code == 0 {
			logger.Info("MEXC Kline WebSocket subscribe success")
		} else {
			logger.Error("MEXC Kline WebSocket subscribe failed: %v", msg)
		}
		return
	}

	// 处理 K线数据
	if channel, ok := msg["channel"].(string); ok && channel == "push.kline" {

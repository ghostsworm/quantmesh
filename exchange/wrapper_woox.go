...
// wooxWrapper WOO X 包装器
type wooxWrapper struct {
	adapter *woox.Adapter
}

// GetName 獲取交易所名称
func (w *wooxWrapper) GetName() string {
	return w.adapter.GetName()
}

func (w *wooxWrapper) GetMarketType() string {
	return w.adapter.GetMarketType()
}

// PlaceOrder 下單
func (w *wooxWrapper) PlaceOrder(ctx context.Context, req *OrderRequest) (*Order, error) {
	var side woox.OrderSide
	if req.Side == SideBuy {
		side = woox.SideBuy
	} else {
		side = woox.SideSell
...
func (w *wooxWrapper) BatchPlaceOrders(ctx context.Context, orders []*OrderRequest) ([]*Order, bool) {
...
// CancelOrder 取消訂單
func (w *wooxWrapper) CancelOrder(ctx context.Context, symbol string, orderID int64) error {
	return w.adapter.CancelOrder(ctx, orderID)
}

// BatchCancelOrders 批量取消訂單
func (w *wooxWrapper) BatchCancelOrders(ctx context.Context, symbol string, orderIDs []int64) error {
	var errs []error
	for _, orderID := range orderIDs {
		errs = appendOrderOpError(errs, orderIDString(orderID), w.adapter.CancelOrder(ctx, orderID))
	}
	return joinOrderOpErrors("batch cancel woox orders", errs)
}

// CancelAllOrders 取消所有订單
func (w *wooxWrapper) CancelAllOrders(ctx context.Context, symbol string) error {
	...
	return joinOrderOpErrors("cancel all woox orders", errs)
}

// GetOrder 查詢訂單
func (w *wooxWrapper) GetOrder(ctx context.Context, symbol string, orderID int64) (*Order, error) {
...
func (w *wooxWrapper) GetOpenOrders(ctx context.Context, symbol string) ([]*Order, error) {
	orders, err := w.adapter.GetOpenOrders(ctx)
	if err != nil {
		return nil, err
	}
...
// GetAccount 獲取帳戶信息
func (w *wooxWrapper) GetAccount(ctx context.Context) (*Account, error) {
...
// GetPositions 獲取持倉
func (w *wooxWrapper) GetPositions(ctx context.Context, symbol string) ([]*Position, error) {
...
// GetBalance 獲取餘額
func (w *wooxWrapper) GetBalance(ctx context.Context, asset string) (float64, error) {
	return w.adapter.GetBalance(ctx)
}

// StartOrderStream 啟動訂單流
func (w *wooxWrapper) StartOrderStream(ctx context.Context, callback func(interface{})) error {
	return w.adapter.StartOrderStream(ctx, callback)
}

// StopOrderStream 停止訂單流
func (w *wooxWrapper) StopOrderStream() error {
	return w.adapter.StopOrderStream()
}

// GetLatestPrice 獲取最新價格
func (w *wooxWrapper) GetLatestPrice(ctx context.Context, symbol string) (float64, error) {
	return w.adapter.GetLatestPrice(ctx, symbol)
}

// StartPriceStream 啟動價格流
func (w *wooxWrapper) StartPriceStream(ctx context.Context, symbol string, callback func(price float64)) error {
...
func (w *wooxWrapper) StartKlineStream(ctx context.Context, symbols []string, interval string, callback CandleUpdateCallback) error {
	return w.adapter.StartKlineStream(ctx, interval, func(candle *woox.CandleLocal) {
		callback(&Candle{
...
func (w *wooxWrapper) StopKlineStream() error {
	return w.adapter.StopKlineStream()
}

// GetHistoricalKlines 獲取歷史 K線
func (w *wooxWrapper) GetHistoricalKlines(ctx context.Context, symbol string, interval string, limit int) ([]*Candle, error) {
...
func (w *wooxWrapper) GetPriceDecimals() int {
	return w.adapter.GetPriceDecimals()
}

// GetQuantityDecimals 獲取數量精度
func (w *wooxWrapper) GetQuantityDecimals() int {
	return w.adapter.GetQuantityDecimals()
}

// GetBaseAsset 獲取基础资產
func (w *wooxWrapper) GetBaseAsset() string {
	return w.adapter.GetBaseAsset()
}

// GetQuoteAsset 獲取报價资產
func (w *wooxWrapper) GetQuoteAsset() string {
	return w.adapter.GetQuoteAsset()
}

// GetFundingRate 獲取资金费率
func (w *wooxWrapper) GetFundingRate(ctx context.Context, symbol string) (float64, error) {
...
func (w *wooxWrapper) InternalTransfer(ctx context.Context, fromAccount, toAccount, asset string, amount float64) (string, error) {

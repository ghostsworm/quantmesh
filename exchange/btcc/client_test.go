...
func TestNewBTCCClient(t *testing.T) {
	apiKey := "test_api_key"
	secretKey := "test_secret_key"

	// 测试主网客户端
	client := NewBTCCClient(apiKey, secretKey, false)
	if client == nil {
		t.Fatal("创建主网客户端失败")
	}
	...
	if client.baseURL != BTCCMainnetBaseURL {
		t.Errorf("主网 URL 错误: 期望 %s, 得到 %s", BTCCMainnetBaseURL, client.baseURL)
	}

	// 测试测试网客户端
	testnetClient := NewBTCCClient(apiKey, secretKey, true)
	if testnetClient.baseURL != BTCCTestnetBaseURL {
		t.Errorf("测试网 URL 错误: 期望 %s, 得到 %s", BTCCTestnetBaseURL, testnetClient.baseURL)
	}
}
...
func TestNewAdapter(t *testing.T) {
	...
	if adapter.GetName() != "BTCC" {
		t.Errorf("交易所名称错误: 期望 BTCC, 得到 %s", adapter.GetName())
	}
}

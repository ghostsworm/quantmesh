...
const (
	PhemexMainnetWSURL = "wss://phemex.com/ws"
	PhemexTestnetWSURL = "wss://testnet.phemex.com/ws"
)

// WebSocketManager Phemex WebSocket 管理器
...
func (w *WebSocketManager) connect(ctx context.Context, symbol string) {
	for {
		...
		if err != nil {
			logger.Error("Phemex WebSocket dial error: %v", err)
		...
		if err := w.authenticate(); err != nil {
			logger.Error("Phemex WebSocket authenticate error: %v", err)
			conn.Close()
			time.Sleep(5 * time.Second)
			continue
		}
		...
		if err := w.subscribe(symbol); err != nil {
			logger.Error("Phemex WebSocket subscribe error: %v", err)
			conn.Close()
			time.Sleep(5 * time.Second)
			continue
		}
		...
		// 连接断开，重连
		logger.Warn("Phemex WebSocket disconnected, reconnecting...")
		time.Sleep(5 * time.Second)
	}
}
...
func (w *WebSocketManager) readMessages() {
	...
	for {
		...
		if err != nil {
			logger.Error("Phemex WebSocket read error: %v", err)
			return
		}
...
func (w *WebSocketManager) handleMessage(message []byte) {
	...
	if err := json.Unmarshal(message, &msg); err != nil {
		logger.Error("Phemex WebSocket unmarshal error: %v", err)
		return
	}

	// 处理认证响应
	if method, ok := msg["method"].(string); ok && method == "user.auth" {
		if result, ok := msg["result"].(map[string]interface{}); ok {
			if status, ok := result["status"].(string); ok && status == "success" {
				logger.Info("Phemex WebSocket authenticated")
			}
		}
		return
	}
	...
	// 处理订单更新
	if method, ok := msg["method"].(string); ok && method == "aop.subscribe" {
		if orders, ok := msg["orders"].([]interface{}); ok {
			logger.Debug("Phemex WebSocket orders update: %d orders", len(orders))
			if w.callback != nil {
				w.callback(orders)
			}
		}

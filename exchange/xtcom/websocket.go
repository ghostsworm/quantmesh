...
const (
	XTMainnetWSURL = "wss://stream.xt.com/public"
	XTTestnetWSURL = "wss://testnet-stream.xt.com/public"
)

// WebSocketManager XT.COM WebSocket 管理器
...
func (w *WebSocketManager) connect(ctx context.Context, symbol string) {
	for {
		...
		if err != nil {
			logger.Error("XT.COM WebSocket dial error: %v", err)
		...
		logger.Info("XT.COM WebSocket connected")

		// 认证
		if err := w.authenticate(); err != nil {
			logger.Error("XT.COM WebSocket authenticate error: %v", err)
			conn.Close()
			time.Sleep(5 * time.Second)
			continue
		}
		...
		if err := w.subscribe(symbol); err != nil {
			logger.Error("XT.COM WebSocket subscribe error: %v", err)
			conn.Close()
			time.Sleep(5 * time.Second)
			continue
		}
		...
		// 连接断开，重连
		logger.Warn("XT.COM WebSocket disconnected, reconnecting...")
		time.Sleep(5 * time.Second)
	}
}
...
func (w *WebSocketManager) readMessages() {
	...
	for {
		...
		if err != nil {
			logger.Error("XT.COM WebSocket read error: %v", err)
			return
		}
...
func (w *WebSocketManager) handleMessage(message []byte) {
	...
	if err := json.Unmarshal(message, &msg); err != nil {
		logger.Error("XT.COM WebSocket unmarshal error: %v", err)
		return
	}
	...
	if method, ok := msg["method"].(string); ok && method == "login" {
		if result, ok := msg["result"].(map[string]interface{}); ok {
			if success, ok := result["success"].(bool); ok && success {
				logger.Info("XT.COM WebSocket authenticated")
			}
		}
		return
	}
	...
	if topic, ok := msg["topic"].(string); ok {
		if data, ok := msg["data"].(interface{}); ok {
			logger.Debug("XT.COM WebSocket message: topic=%s", topic)
			if w.callback != nil {
				w.callback(data)
			}
		}

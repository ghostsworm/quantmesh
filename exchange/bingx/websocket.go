package bingx

import (
	"context"
	"crypto/hmac"
	"crypto/sha256"
	"encoding/hex"
	"encoding/json"
	"fmt"
	"strconv"
	"sync"
	"time"

	"quantmesh/logger"

	"github.com/gorilla/websocket"
)

const (
	BingXMainnetWSURL = "wss://open-api-swap.bingx.com/swap-market"
	BingXTestnetWSURL = "wss://open-api-vst.bingx.com/swap-market"
)

// WebSocketManager BingX WebSocket 管理器
type WebSocketManager struct {
	apiKey    string
	secretKey string
	wsURL     string
	conn      *websocket.Conn
	mu        sync.RWMutex
	stopCh    chan struct{}
	callback  func(interface{})
	isRunning bool
}

// NewWebSocketManager 创建 WebSocket 管理器
func NewWebSocketManager(apiKey, secretKey string, isTestnet bool) *WebSocketManager {
	wsURL := BingXMainnetWSURL
	if isTestnet {
		wsURL = BingXTestnetWSURL
	}

	return &WebSocketManager{
		apiKey:    apiKey,
		secretKey: secretKey,
		wsURL:     wsURL,
		stopCh:    make(chan struct{}),
	}
}

// Start 启动 WebSocket
func (w *WebSocketManager) Start(ctx context.Context, symbol string, callback func(interface{})) error {
	w.mu.Lock()
	if w.isRunning {
		w.mu.Unlock()
		return fmt.Errorf("websocket already running")
	}
	w.callback = callback
	w.isRunning = true
	w.mu.Unlock()

	go w.connect(ctx, symbol)
	return nil
}

// Stop 停止 WebSocket
func (w *WebSocketManager) Stop() {
	w.mu.Lock()
	defer w.mu.Unlock()

	if !w.isRunning {
		return
	}

	w.isRunning = false
	close(w.stopCh)

	if w.conn != nil {
		w.conn.Close()
	}
}

// connect 连接 WebSocket
func (w *WebSocketManager) connect(ctx context.Context, symbol string) {
	for {
		select {
		case <-w.stopCh:
			return
		default:
		}

		conn, _, err := websocket.DefaultDialer.Dial(w.wsURL, nil)
		if err != nil {
			logger.Error("BingX WebSocket dial error: %v", err)
			time.Sleep(5 * time.Second)
			continue
		}

		w.mu.Lock()
		w.conn = conn
		w.mu.Unlock()

		logger.Info("BingX WebSocket connected")

		// 认证
		if err := w.authenticate(); err != nil {
			logger.Error("BingX WebSocket authenticate error: %v", err)
			conn.Close()
			time.Sleep(5 * time.Second)
			continue
		}

		// 订阅频道
		if err := w.subscribe(symbol); err != nil {
			logger.Error("BingX WebSocket subscribe error: %v", err)
			conn.Close()
			time.Sleep(5 * time.Second)
			continue
		}

		// 启动心跳
		go w.heartbeat()

		// 读取消息
		w.readMessages()

		// 连接断开，重连
		logger.Warn("BingX WebSocket disconnected, reconnecting...")
		time.Sleep(5 * time.Second)
	}
}

// authenticate 认证
func (w *WebSocketManager) authenticate() error {
	timestamp := strconv.FormatInt(time.Now().UnixMilli(), 10)

	// 签名：HMAC-SHA256(apiKey + timestamp)
	signStr := w.apiKey + timestamp
	h := hmac.New(sha256.New, []byte(w.secretKey))
	h.Write([]byte(signStr))
	signature := hex.EncodeToString(h.Sum(nil))

	authMsg := map[string]interface{}{
		"id":       "auth",
		"reqType":  "auth",
		"dataType": timestamp,
		"data": map[string]string{
			"apiKey":    w.apiKey,
			"timestamp": timestamp,
			"signature": signature,
		},
	}

	return w.sendMessage(authMsg)
}

// subscribe 订阅频道
func (w *WebSocketManager) subscribe(symbol string) error {
	// 订阅账户更新
	subMsg := map[string]interface{}{
		"id":       "account_sub",
		"reqType":  "sub",
		"dataType": "account.update",
	}

	if err := w.sendMessage(subMsg); err != nil {
		return err
	}

	// 订阅订单更新
	subMsg = map[string]interface{}{
		"id":       "order_sub",
		"reqType":  "sub",
		"dataType": "order.update",
	}

	if err := w.sendMessage(subMsg); err != nil {
		return err
	}

	// 订阅 Ticker
	subMsg = map[string]interface{}{
		"id":       "ticker_sub",
		"reqType":  "sub",
		"dataType": symbol + "@ticker",
	}

	return w.sendMessage(subMsg)
}

// sendMessage 发送消息
func (w *WebSocketManager) sendMessage(msg interface{}) error {
	w.mu.RLock()
	conn := w.conn
	w.mu.RUnlock()

	if conn == nil {
		return fmt.Errorf("websocket not connected")
	}

	data, err := json.Marshal(msg)
	if err != nil {
		return err
	}

	return conn.WriteMessage(websocket.TextMessage, data)
}

// heartbeat 心跳
func (w *WebSocketManager) heartbeat() {
	ticker := time.NewTicker(20 * time.Second)
	defer ticker.Stop()

	for {
		select {
		case <-w.stopCh:
			return
		case <-ticker.C:
			pingMsg := map[string]interface{}{
				"id":      "ping",
				"reqType": "ping",
			}
			if err := w.sendMessage(pingMsg); err != nil {
				logger.Error("BingX WebSocket ping error: %v", err)
				return
			}
		}
	}
}

// readMessages 读取消息
func (w *WebSocketManager) readMessages() {
	w.mu.RLock()
	conn := w.conn
	w.mu.RUnlock()

	if conn == nil {
		return
	}

	for {
		select {
		case <-w.stopCh:
			return
		default:
		}

		_, message, err := conn.ReadMessage()
		if err != nil {
			logger.Error("BingX WebSocket read error: %v", err)
			return
		}

		w.handleMessage(message)
	}
}

// handleMessage 处理消息
func (w *WebSocketManager) handleMessage(message []byte) {
	var msg map[string]interface{}
	if err := json.Unmarshal(message, &msg); err != nil {
		logger.Error("BingX WebSocket unmarshal error: %v", err)
		return
	}

	// 处理 pong
	if reqType, ok := msg["reqType"].(string); ok && reqType == "pong" {
		return
	}

	// 处理认证响应
	if id, ok := msg["id"].(string); ok && id == "auth" {
		if code, ok := msg["code"].(float64); ok && code == 0 {
			logger.Info("BingX WebSocket auth success")
		} else {
			logger.Error("BingX WebSocket auth failed: %v", msg)
		}
		return
	}

	// 处理订阅响应
	if reqType, ok := msg["reqType"].(string); ok && reqType == "sub" {
		if code, ok := msg["code"].(float64); ok && code == 0 {
			logger.Info("BingX WebSocket subscribe success")
		} else {
			logger.Error("BingX WebSocket subscribe failed: %v", msg)
		}
		return
	}

	// 处理数据推送
	if dataType, ok := msg["dataType"].(string); ok {
		if data, ok := msg["data"].(interface{}); ok {
			logger.Debug("BingX WebSocket message: dataType=%s, data=%v", dataType, data)
			if w.callback != nil {
				w.callback(data)
			}
		}
	}
}

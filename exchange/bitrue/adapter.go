...
// convertSymbolToBitrue 轉换交易對格式：BTCUSDT -> BTCUSDT
...
func (a *Adapter) GetLatestPrice(ctx context.Context, symbol string) (float64, error) {
	...
	if symbol != "" {
		targetSymbol = convertSymbolToBitrue(symbol)
	}
...
// GetFundingRate 獲取资金费率（Bitrue 現貨交易所，回傳 0）

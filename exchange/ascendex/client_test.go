...
func TestNewAscendEXClient(t *testing.T) {
	...
	if client.baseURL != AscendEXMainnetBaseURL {
		t.Errorf("主网 URL 錯误: 期望 %s, 得到 %s", AscendEXMainnetBaseURL, client.baseURL)
	}

	// 测試測試網客戶端
	testnetClient := NewAscendEXClient(apiKey, secretKey, true)
	if testnetClient.baseURL != AscendEXTestnetBaseURL {
		t.Errorf("測試網 URL 錯误: 期望 %s, 得到 %s", AscendEXTestnetBaseURL, testnetClient.baseURL)
	}
}
...
func TestSignRequest(t *testing.T) {
	...
	// 驗证签名长度（HMAC-SHA256 + Base64 应該產生 44 字符的字符串）
	if len(signature) == 0 {
		t.Error("签名长度不能為 0")
	}
...
func TestNewAdapter(t *testing.T) {
	...
	if adapter.GetName() != "AscendEX" {
		t.Errorf("交易所名称錯误: 期望 AscendEX, 得到 %s", adapter.GetName())
	}
}

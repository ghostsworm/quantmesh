/**
 * Translate webui i18n locale files using Gemini API (chunked by top-level keys).
 ...
 *   GEMINI_API_KEY=your_key node webui/scripts/translate-locales.js zh-TW fr-FR
 */
...
async function translateChunk(apiKey, chunkJson, targetLocale, chunkName) {
  const langName = LOCALE_NAMES[targetLocale] || targetLocale
  const prompt = `You are a precise translator. Translate ONLY the string values in the following JSON to ${langName}. Keep every key unchanged. Preserve nested structure and placeholders like {{.Version}}, {{count}}, {{exchange}}, {{symbol}}, etc. Return only valid JSON, no explanation, no markdown.
...
${chunkJson}`
  ...
  try {
    ...
    throw new Error(`Invalid JSON for ${chunkName}: ${e.message}`)
  }
}
...
async function main() {
  ...
  for (const locale of locales) {
    ...
    const result = {}
    console.log('Translating', locale, `(${topKeys.length} chunks)...`)
    for (const key of topKeys) {
      const chunk = { [key]: source[key] }
      const chunkStr = JSON.stringify(chunk)
      try {
        const translated = await translateChunk(apiKey, chunkStr, locale, key)
        Object.assign(result, translated)
        process.stdout.write('.')
        await new Promise((r) => setTimeout(r, 300))
      } catch (e) {
        console.error('\nFailed', locale, key, e.message)
      }
    }
    if (Object.keys(result).length === topKeys.length) {
      fs.writeFileSync(outPath, JSON.stringify(result, null, 2) + '\n', 'utf8')
      console.log('\nWritten:', outPath)
    } else {
      console.log('\nSkipped writing', locale, '(incomplete)')
    }
  }
  console.log('Done.')
}

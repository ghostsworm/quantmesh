...
export interface CapitalOverview {
  ...
  usedCapital: number // 實際已占用保证金
  availableCapital: number // 交易所可用餘額
  reservedCapital: number // 用戶預留资金
...
export interface StrategyCapitalInfo {
  ...
  exchangeId: string
  asset: string
  allocated: number // 分配金額
  used: number // 已使用
  available: number // 可用配額
  weight: number // 权重
  maxCapital?: number // 最大限額
  maxPercentage?: number // 最大占比
  reserveRatio?: number // 預留保证金比例

...
const GlobalDashboard: React.FC = () => {
      ...
      try {
        ...
        for (const sym of symbolsData.symbols) {
          try {
            ...
            console.error(t('globalDashboard.fetchStatusFailed', { exchange: sym.exchange, symbol: sym.symbol }), err)
          }
        }
        setSymbolStatuses(statusMap)
        setLoading(false)
      } catch (error) {
        console.error(t('globalDashboard.fetchGlobalDataFailed'), error)
        toast({
  ...
  const chartData = useMemo(() => {
    ...
    return dailyStats.map(d => ({
      time: new Date(d.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }),
      pnl: d.total_pnl
    })).reverse()
  }, [dailyStats])
    ...
    return (
      <Center h="calc(100vh - 100px)">
        <VStack spacing={4}>
          ...
          <Text color="gray.500" fontSize="sm" fontWeight="600">{t('globalDashboard.building')}</Text>
        </VStack>
      </Center>
    )
  }
    ...
    <Box minH="100vh" py={2}>
      <VStack align="stretch" spacing={8}>
        <Flex justify="space-between" align="flex-end" px={2}>
          <Box>
            <Heading size="lg" fontWeight="800" mb={1}>{t('globalDashboard.title')}</Heading>
            <Text color="gray.500" fontSize="sm">{t('globalDashboard.subtitle')}</Text>
          </Box>
        ...
        >
          <Flex justify="space-between" align="flex-start" mb={6} direction={{ base: 'column', sm: 'row' }} gap={4}>
            <VStack align="start" spacing={0}>
              <Text color="gray.500" fontSize="xs" fontWeight="bold" textTransform="uppercase">{t('globalDashboard.totalPnLTrend')}</Text>
            ...
            <HStack spacing={6} alignSelf={{ base: 'flex-start', sm: 'flex-end' }}>
              <Stat size="sm">
                <StatLabel color="gray.500" fontSize="xs" fontWeight="bold">{t('globalDashboard.activeSymbols')}</StatLabel>
                <StatNumber fontSize="lg" fontWeight="800">{summary.activeCount} / {summary.totalCount}</StatNumber>
              </Stat>
              <Stat size="sm">
                <StatLabel color="gray.500" fontSize="xs" fontWeight="bold">{t('globalDashboard.totalTrades')}</StatLabel>
        ...
        <SimpleGrid columns={{ base: 1, md: 2, lg: 4 }} spacing={6}>
          <DashboardCard title={t('globalDashboard.totalVolume')} icon={RepeatIcon} helpText={t('globalDashboard.totalVolumeHelp')}>
            <Heading size="md" fontWeight="800">${summary.totalVolume.toLocaleString()}</Heading>
            <Text fontSize="xs" color="gray.500" mt={1}>{t('globalDashboard.last24Hours')}</Text>
          </DashboardCard>
          <DashboardCard title={t('globalDashboard.winRate')} icon={CheckCircleIcon}>
            <Heading size="md" fontWeight="800">68.4%</Heading>
            <Text fontSize="xs" color="green.500" mt={1}>{t('globalDashboard.winRateChange')}</Text>
          </DashboardCard>
          <DashboardCard title={t('globalDashboard.riskLevel')} icon={WarningIcon}>
            <Heading size="md" color="green.500" fontWeight="800">{t('globalDashboard.safe')}</Heading>
            <Text fontSize="xs" color="gray.500" mt={1}>{t('globalDashboard.allThresholdsInRange')}</Text>
          </DashboardCard>
          <DashboardCard title={t('globalDashboard.apiLatency')} icon={TimeIcon}>
            <Heading size="md" fontWeight="800">42ms</Heading>
            <Text fontSize="xs" color="gray.500" mt={1}>{t('globalDashboard.connectedToBinanceAPAC')}</Text>
          </DashboardCard>
        </SimpleGrid>

        {/* 交易对列表 */}
        <Box>
          <Heading size="md" mb={6} px={2}>{t('globalDashboard.tradingPairMatrix')}</Heading>
                  ...
                  >
                    <Flex justify="space-between" align="center" mb={4}>
                      <VStack align="start" spacing={0}>
                        ...
                        <Text color="gray.500" fontSize="xs">{t('globalDashboard.recentPrice')}: ${sym.current_price.toFixed(2)}</Text>
                      </VStack>
                      <Box
                        w={3}
                        h={3}
                    ...
                    {status && (
                      <SimpleGrid columns={2} spacing={4}>
                        <Box>
                          <Text color="gray.400" fontSize="10px" fontWeight="bold" textTransform="uppercase">{t('globalDashboard.todayPnL')}</Text>
                          <Text color={status.total_pnl >= 0 ? 'green.500' : 'red.500'} fontWeight="800" fontSize="md">
                            {status.total_pnl >= 0 ? '+' : ''}{status.total_pnl.toFixed(2)}
                          </Text>
                        </Box>
                        <Box>
                          <Text color="gray.400" fontSize="10px" fontWeight="bold" textTransform="uppercase">{t('globalDashboard.tradeCount')}</Text>

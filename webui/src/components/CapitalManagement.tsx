    ...
    try {
      ...
      const allocations: CapitalAllocationConfig[] = Object.entries(pendingChanges)
        ...
        .map(([key, maxCapital]) => {
          ...
          return {
            strategyId, // 发送纯 strategyId，后端會处理
            maxCapital: Math.max(0, maxCapital), // 确保不為负數
            maxPercentage: Math.min(100, Math.max(0, maxPercentage)), // 确保在 0-100 範圍内
              ...
              <VStack align="stretch" spacing={3}>
                {currentStrategies && currentStrategies.length > 0 ? (
                  currentStrategies
                    .filter(s => s !== null && s !== undefined)
                    .map((strategy, index) => {
                      // 生成唯一 key：結合 exchangeId, strategyId, asset 和 index 确保唯一性

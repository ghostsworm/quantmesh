...
const STRATEGY_TEMPLATES = {
  ...
  balanced: {
    name: '平衡型',
    description: '中等风險，适中收益，适合有經驗的交易者',
...
const STRATEGY_TYPES = [
  {
    type: 'dca',
    name: '增强型 DCA',
    description: 'ATR动態间距、三重止盈、多层倉位管理',
    icon: <Timeline />,
    features: ['动態间距', '三重止盈', '瀑布保护'],
  },
  {
    type: 'martingale',
    name: '马丁格尔',
    description: '加倍加倉、风險遞减控制',
    icon: <TrendingDown />,
    features: ['加倍加倉', '风險遞减', '反向马丁'],
  },
  {
    type: 'combo',
    name: '组合策略',
    description: '多空對冲、市况自适应',
    icon: <AccountBalance />,
    features: ['多空對冲', '自适应权重', '全時况覆盖'],
  },
  {
    type: 'trend',
    name: '趋势跟踪',
    description: '顺势交易、动態止盈',
    icon: <TrendingUp />,
    features: ['趋势识别', '动態止盈', '移动止损'],
  },
];
...
const StrategyWizard: React.FC<StrategyWizardProps> = ({
  ...
  // 步骤定义
  const steps = [
    { label: '选擇风格', description: '选擇您的交易风格' },
    { label: '选擇策略', description: '选擇策略類型' },
    { label: '配置参數', description: '調整策略参數' },
    { label: '风險评估', description: '查看风險评估結果' },
    { label: '确认啟动', description: '确认並啟动策略' },
  ];
  ...
  const runRiskAssessment = useCallback(async () => {
    ...
    try {
      ...
      if (!parameters.stopLoss || parameters.stopLoss <= 0) {
        score -= 25;
        warnings.push('未設置止损：强烈建议設置止损保护');
        suggestions.push({
          title: '添加止损設置',
          description: '建议設置5-15%的止损比例',
          priority: 'high',
        });
          ...
          <Box>
            ...
            <Paper sx={{ p: 3, mb: 3 }}>
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={6}>
                  <Typography gutterBottom>杠杆倍數: {parameters.leverage}x</Typography>
                  <Slider
            ...
            <Paper sx={{ p: 3, mb: 3 }}>
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={6}>
                  <Typography gutterBottom>最大层數: {parameters.maxLayers}</Typography>
                  <Slider
                ...
                <Grid item xs={12} md={6}>
                  <Typography gutterBottom>價格间距: {parameters.priceStep}%</Typography>
                  <Slider
            ...
            {isAssessing ? (
              ...
              <Box>
                <Paper sx={{ p: 3, mb: 3, textAlign: 'center' }}>
                  ...
                  <Chip
                    label={
                      riskAssessment.riskLevel === 'low' ? '低风險' :
                      riskAssessment.riskLevel === 'medium' ? '中等风險' :
                      riskAssessment.riskLevel === 'high' ? '高风險' : '极高风險'
                    }
        ...
        return (
          <Box>
            ...
            <Paper sx={{ p: 3, mb: 3 }}>
              <Grid container spacing={2}>
                <Grid item xs={6}>
                  <Typography color="text.secondary">策略類型</Typography>
                  <Typography variant="h6">
                    {STRATEGY_TYPES.find(s => s.type === strategyType)?.name}
                  </Typography>
                </Grid>
                <Grid item xs={6}>
                  <Typography color="text.secondary">交易對</Typography>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={4}>
                  <Typography color="text.secondary">最大层數</Typography>
                  <Typography>{parameters.maxLayers}</Typography>
                </Grid>
                <Grid item xs={4}>
                  <Typography color="text.secondary">價格间距</Typography>
                  <Typography>{parameters.priceStep}%</Typography>
                </Grid>
                <Grid item xs={4}>
                  <Typography color="text.secondary">趨勢過濾</Typography>
                  <Typography>{parameters.trendFilter ? '啟用' : '禁用'}</Typography>
                </Grid>
              </Grid>
            </Paper>
            ...
            {riskAssessment && (
              ...
              >
                风險评分: {riskAssessment.overallScore}/100 - 
                {riskAssessment.recommended ? ' 建议可以啟动此策略' : ' 建议先优化配置'}
              </Alert>
            )}
          </Box>
        );

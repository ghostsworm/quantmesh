  ...
  const fetchCurrentBasis = async () => {
    try {
      ...
      if (isServiceUnavailable(err)) {
        ...
        setError('價差監控服務未啟用。请在配置文件中啟用價差監控功能。');
      } else {
        setError(err instanceof Error ? err.message : '獲取價差數據失败');
        setServiceUnavailable(false);
      }
    }
  };
    ...
    try {
      ...
      if (isServiceUnavailable(err)) {
        ...
        if (!serviceUnavailable) {
          setError(err instanceof Error ? err.message : '獲取歷史數據失败');
        }
      }
    } finally {
      setLoading(false);
  ...
  const fetchStatistics = async (symbol: string, hours: number = 24) => {
    try {
      ...
      if (isServiceUnavailable(err)) {
        ...
        if (!serviceUnavailable) {
          setError(err instanceof Error ? err.message : '獲取统计數據失败');
        }
      }
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold">現貨-合約價差監控</h1>
      ...
      {error && (
        <div className={`border px-4 py-3 rounded ${
          ...
          <div className="font-semibold mb-2">
            {serviceUnavailable ? '⚠️ 價差監控服務未啟用' : '❌ 錯误'}
          </div>
          <div className="text-sm">{error}</div>
          {serviceUnavailable && (
            <div className="mt-3 text-sm">
              <p className="font-semibold mb-1">啟用方法：</p>
...
{`basis_monitor:
  ...
  symbols:
    ...
    - ETHUSDT`}
              ...
              <p className="mt-2 text-xs">修改后请重啟服務以使配置生效。</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-xl font-semibold mb-4">實時價差</h2>
            ...
            >
              ...
              <div className="space-y-1 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-600">現貨:</span>
                  <span>${formatPrice(data.spot_price)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">合約:</span>
                  <span>${formatPrice(data.futures_price)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">價差:</span>
      ...
      {statistics && (
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-xl font-semibold mb-4">
            {selectedSymbol} 统计數據 (最近 {statistics.hours} 小時)
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div className="text-center p-4 bg-gray-50 rounded">
              <div className="text-gray-600 text-sm">平均價差</div>
              <div className="text-lg font-semibold">
                {formatPercent(statistics.avg_basis)}
              </div>
            </div>
            <div className="text-center p-4 bg-gray-50 rounded">
              <div className="text-gray-600 text-sm">最大價差</div>
              <div className="text-lg font-semibold text-red-600">
                {formatPercent(statistics.max_basis)}
              </div>
            </div>
            <div className="text-center p-4 bg-gray-50 rounded">
              <div className="text-gray-600 text-sm">最小價差</div>
              <div className="text-lg font-semibold text-green-600">
                {formatPercent(statistics.min_basis)}
              </div>
            </div>
            <div className="text-center p-4 bg-gray-50 rounded">
              <div className="text-gray-600 text-sm">標准差</div>
              <div className="text-lg font-semibold">
                {statistics.std_dev.toFixed(4)}%
              </div>
            </div>
            <div className="text-center p-4 bg-gray-50 rounded">
              <div className="text-gray-600 text-sm">數據点</div>
              <div className="text-lg font-semibold">
      ...
      {/* 历史图表 */}
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-xl font-semibold mb-4">價差历史趋势</h2>
        {loading ? (
          <div className="text-center py-8">加載中...</div>
        ) : history.length > 0 ? (
          <Line data={chartData} options={chartOptions} />
        ) : (
          <div className="text-center py-8 text-gray-500">暂無历史數據</div>
        )}
      </div>

      {/* 說明 */}
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <h3 className="font-semibold mb-2">💡 價差說明</h3>
        <ul className="text-sm space-y-1 text-gray-700">
          <li>• <strong>正價差</strong>（合約價格 &gt; 現貨價格）：市场看涨，可考虑正向套利（買現貨+空合約）</li>
          <li>• <strong>负價差</strong>（合約價格 &lt; 現貨價格）：市场看跌，可考虑反向套利（空現貨+買合約）</li>
          <li>• <strong>资金费率</strong>：正费率表示多头支付空头，负费率表示空头支付多头</li>
          <li>• 價差與资金费率通常呈正相关，但背离時可能存在套利机會</li>
        </ul>
      </div>
    </div>
  );

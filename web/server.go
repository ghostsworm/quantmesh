	...
	api.Use(versionHeaderMiddleware())
	{
		...
		protected.Use(authMiddleware())
		{
			...
			fixGroup.Use(fixEnabledMiddleware())
			{
				...
				fixGroup.GET("/orders", getFixOrderLinks)
				fixGroup.POST("/sessions/logon", fixLogonSession)
				fixGroup.POST("/sessions/heartbeat", fixHeartbeat)
				fixGroup.POST("/sessions/logout", fixLogoutSession)
				fixGroup.POST("/orders/new", fixNewOrder)
				fixGroup.POST("/orders/cancel", fixCancelOrder)
				fixGroup.POST("/orders/replace", fixReplaceOrder)
			}

...
func getMarketTicker(c *gin.Context) {
	...
	switch exchangeName {
	case "binance":
		markPrice, lastPrice, high24h, low24h, err = fetchBinanceTicker(symbol, marketType)
	case "bitget":
		markPrice, lastPrice, high24h, low24h, err = fetchBitgetTicker(symbol, marketType)
	case "bybit":
		markPrice, lastPrice, high24h, low24h, err = fetchBybitTicker(symbol, marketType)
	case "okx":
		markPrice, lastPrice, high24h, low24h, err = fetchOKXTicker(symbol, marketType)
	default:
		markPrice, lastPrice, high24h, low24h, err = fetchBinanceTicker(symbol, marketType)
	}
	if err != nil {
...
func fetchBinanceTicker(symbol, marketType string) (mark, last, high24h, low24h float64, err error) {
	if marketType == "spot" {
	...
	if e := json.Unmarshal(body24, &data24); e != nil {
	...
	respMark, e := http.Get(urlMark)
	if e != nil {
		mark = last
		return mark, last, high24h, low24h, nil
	}
	defer respMark.Body.Close()
	bodyMark, _ := io.ReadAll(respMark.Body)
	var dataMark struct {
		MarkPrice string `json:"markPrice"`
	}
	if e := json.Unmarshal(bodyMark, &dataMark); e == nil && dataMark.MarkPrice != "" {
...
func fetchBitgetTicker(symbol, marketType string) (mark, last, high24h, low24h float64, err error) {
	if marketType == "spot" {
		...
		if e := json.Unmarshal(body, &data); e != nil || len(data.Data) == 0 {
...
func fetchBybitTicker(symbol, marketType string) (mark, last, high24h, low24h float64, err error) {
	category := "linear"
	if marketType == "spot" {
		category = "spot"
	}
	...
	if e := json.Unmarshal(body, &data); e != nil || len(data.Result.List) == 0 {
...
func fetchOKXTicker(symbol, marketType string) (mark, last, high24h, low24h float64, err error) {
	var instId string
	if marketType == "spot" {
		if len(symbol) > 4 {
